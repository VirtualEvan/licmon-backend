...
def parse_product(stdout):
    ...
    regex_feature = re.compile('Users of (.*?):  \(Total of (\d+) licenses? issued;  Total of (\d+) licenses? in use\)')
    ...
    regex_user = re.compile('(\w+) (.*) (.*) (?:\(v(.*)\))? \((.*)\/(\d+) (\d+)\), start (\w+ \d+\/\d+ \d+\:\d+)(?:, (\d+) licenses)?')

    # Users of visualhdlpro_c:  (Error: 5 licenses, unsupported by licensed server)
    regex_error_unsupported = re.compile('Users of (\S+):\s*\(Error: (\d+) licenses, (unsupported by licensed server)\)')

    # lmgrd is not running: License server machine is down or not responding.
    regex_error_down = re.compile('lmgrd is not running: License server machine is down or not responding.')
